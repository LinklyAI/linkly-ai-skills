...
| `--type <types>`          | Filter by document type name, comma-separated — `pdf`, `md`, `xlsx`, `csv`, `doc`, `txt`, `docx`, `pptx`, `rtf`, `epub`, `html`, `image`, `audio`, `video` (e.g. `pdf,md`). Type name, not extension.                            |
