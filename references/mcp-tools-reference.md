...
- Pure content/topic queries ("find resumes", "find AI papers") — call `search` directly; unfiltered hybrid retrieval already covers title/filename/content/path.
...
| `readme`     | `object`         | Pointer to the listed directory's README-style file: `{ doc_id, path, word_count, total_lines, skip_reason }` — a pointer, not inlined content. When present and you need to understand the folder's purpose, `read` it first. Emitted only when **all** conditions below hold; otherwise the field is absent entirely. |
...
**`readme` emission conditions** — a README existing on disk is not enough; every condition must hold or the field is absent:

- Only the **direct children** of an explicitly passed local `path` are checked (scope `folder`, or a **local** library with `path`) — never cloud libraries, never the no-`path` all-roots sweep.
- Best whitelist hit wins (basenames matched case-insensitively), in priority order: `README.md` > `README.txt` > `index.md` > `_index.md` > folder-note (`<directory name>.md`).
- The file must already have an index row (a `doc_id` exists) with `skip_reason` null — otherwise the pointer would be a dead link for `read`.
- Agent-instruction files are blacklisted after every whitelist channel, folder-note included: `claude.md`, `agents.md`, `agent.md`, `gemini.md`, `.cursorrules`, `.windsurfrules`, `copilot-instructions.md` — so e.g. `CLAUDE/CLAUDE.md` never qualifies.

Each item: `doc_id` (for `read`/`grep`/`outline`; null for an unindexed note), `title` (null for machine-named notes), `path` (absolute for local scopes; root-relative for cloud), `doc_type`, `word_count`, `total_lines`, `has_outline`, `modified_at` (Unix ms; filesystem mtime locally / D1 mtime in cloud — nullable for cloud), `keywords`, `snippet`, `skip_reason` (non-null = content not readable — don't `read` it). Notes items additionally carry `note_id`, `version` (the CAS pair for `note_save` mode="edit"), `created_at`, `tags`, `source`/`agent`/`updated_by` provenance (`agent` is null unless the note came from an agent), and `indexed`. For notes, `indexed: false` always means `doc_id: null`, `word_count: null`, `total_lines: null`, `has_outline: false`, and `keywords: []`; wait for a later `list` result with `indexed: true` and a non-null `doc_id` before calling `read`/`grep`/`outline`.
...
| `time_sort`       | `string`   | No       | `"default"` | One of `"default"` / `"newest"` / `"oldest"`. `"default"` preserves the backend relevance order (hybrid when eligible; otherwise winner-first BM25). `"newest"` / `"oldest"` reorder by `modified_at` after dedup, useful for "latest / earliest".                                                                                                              |
...
| `relevance`   | `number`   | Backend relevance score, rendered to 2 decimals: hybrid BM25 + vector when eligible, or winner-first BM25 for cloud searches with path/type/time filters. Higher = more relevant. Not normalized to a fixed range — use it for ordering, not as a 0–1 threshold.                         |
