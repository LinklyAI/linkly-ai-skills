...
- **MCP**: check whether MCP tools named `search`, `find_paths`, `list`, `outline`, `grep`, `read`, `list_libraries`, `explore`, and `note_save` are accessible in the current environment. The `linkly-ai` server (local Desktop MCP) exposes exactly those nine. The `linkly-ai-cloud` server (the `mcp.linkly.ai` cloud gateway) exposes the same nine **plus three cloud-only tools** — `search_libraries`, `link_library` and `unlink_library` — that discover, link and unlink cloud libraries; they are never advertised on a local or LAN connection. Beyond those three, the difference is reach, not the tool list — see "Know what your connection reaches" below. `note_save` is the one tool whose reach never varies: it always resolves to the user's Desktop, whichever server it arrived from.

### 2. Pick a mode

| Available                                       | Action                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CLI reachable + MCP**                         | Prefer **CLI mode** — clearer error messages and exit codes are easier to surface back to the user.                                                                                                                                                                                                                                                              |
| **CLI reachable only**                          | Use **CLI mode**.                                                                                                                                                                                                                                                                                                                                                |
| **CLI unreachable + MCP**                       | Use **MCP mode**. The CLI binary exists, but its localhost path may be blocked by the shell sandbox. Do not claim the Desktop is stopped and do not keep retrying the sandboxed CLI.                                                                                                                                                                             |
| **CLI installed but unreachable, no MCP tools** | If the command is running in an AI-agent network sandbox, ask the user to approve retrying the **exact Linkly CLI command** outside that sandbox. Do not ask them to disable sandboxing globally or request broad approval for every `linkly` subcommand. Only if the approved retry also fails should you ask the user to launch Linkly AI Desktop, then retry. |
| **MCP only**                                    | Use **MCP mode**. This is normal in restricted agent environments where the CLI binary is not installed inside the shell. Don't tell the user to install the CLI; MCP is sufficient.                                                                                                                                                                             |
| **Neither installed**                           | If Bash works and `linkly --version` failed, recommend installing the CLI: [Install Linkly AI CLI](https://linkly.ai/docs/en/use-cli). Otherwise inform the user that Linkly AI requires either the CLI or the MCP integration and stop.                                                                                                                         |
...
| **Cloud gateway only** — the `search_libraries` / `link_library` / `unlink_library` tools         | The cloud library **catalog** (including libraries not linked yet) and the link / unlink actions. Not advertised on Local / LAN — if the user needs them there, tell them to switch connection. |
...
**Notes sit outside this table.** They are local files with no cloud counterpart, so `note_save`, `list` with `scope="notes"` and `search` with `scope="notes"` always reach the Desktop no matter which connection you are on — over the tunnel when you are on the cloud gateway. Consequences: they need the Desktop online (over the tunnel, that also means Pro), and when it is unreachable there is **no cloud fallback to retry against** — do not "retry with a cloud library", there is nothing there to retry against. Never pass a `library` alongside `scope="notes"`, or to `note_save` at all — notes are a single local container and the call is rejected.
...
- Add `--type` filter when the user mentions a specific format (`pdf`, `md`, `xlsx`, `csv`, `docx`, `txt`, `pptx`, `doc`, `rtf`, `epub`, `html`, `image`, `audio`, `video`). Audio and video match against their transcripts; images and scanned PDFs against their OCR text; spreadsheets against their cell text.
...
⚠️ `scope="notes"` **ignores `path_glob` and a local `library`** — passing them together silently drops that filter rather than erroring. Pairing it with a `cloud://` library is a different case: that one is **rejected outright**, because notes have no cloud counterpart and silently searching the wrong place would hand you an empty result you'd read as "the user has no such notes".
...
Call `list_libraries` to see both kinds and their identifiers — it lists everything that is **searchable right now**. A cloud library that is **not linked yet** does not appear there. On the cloud gateway, find it with `search_libraries` (returns the exact `cloud://owner/slug`, plus `is_linked` and `can_link`), link it with `link_library`, then search it. Those two tools are the only way to reach an unlinked library from an agent; on Local / LAN there is no way at all. `search_libraries` finds *libraries*; `search` finds *documents* — never use one for the other.

### When to use libraries

- **User explicitly names a local library:** "search in my-research library" → `--library my-research`
- **User names a cloud library:** find it in `list_libraries`, then scope with `library="cloud://<owner>/<slug>"`. If it is not listed and you are on the cloud gateway: `search_libraries` → `link_library` first. Public libraries allow any signed-in user to link; Showcase / Private libraries allow their owner or readers who have accepted an invitation to link. For other users, the tool answers `invite_required` for Showcase libraries, or `not_found` for private libraries they cannot see.
- **User wants a cloud library by topic, not by name** ("find a Rust knowledge base and use it"): `search_libraries(query="Rust")` → pick a result → `link_library(library="cloud://<owner>/<slug>")` → `search` inside it. Do **not** answer this with `search` — that searches documents, not the catalog
- **`link_library` answers `slot_exhausted`** (`data.kind`): the account's link quota is full (Free 1 / Pro 99). Report the numbers to the user. If the user has already named a linked library to give up ("replace A with B"), call `unlink_library` on it, then `link_library` again. Otherwise call `list_libraries`, show the linked cloud libraries and ask which one to release — **never choose one yourself**, and never unlink to make room without being told which. Unlinking is a destructive write: it needs the user's word on the specific library, the same bar as writing configuration. The website and, on Free, the upgrade link remain the alternatives
...
- On a **cloud gateway** connection: call `list_libraries` to see what is linked, then issue one `search` per relevant cloud library. If the library the user means is not linked yet, `search_libraries` → `link_library` first.
