...
| **Cloud gateway only** — the `search_libraries` / `link_library` tools                            | The cloud library **catalog** (including libraries not linked yet) and the link action. Not advertised on Local / LAN — if the user needs them there, tell them to switch connection. |
...
- **`link_library` answers `slot_exhausted`** (`data.kind`): the account's link quota is full (Free 1 / Pro 99). Report the numbers and the upgrade link to the user and stop. **Never unlink another library to make room** — that decision belongs to the user, on the website
