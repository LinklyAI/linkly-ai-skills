...
- Add `--type` filter when the user mentions a specific format (`pdf`, `md`, `xlsx`, `csv`, `doc`, `txt`, `docx`, `pptx`, `rtf`, `epub`, `html`, `image`, `audio`, `video`). Audio and video match against their transcripts; images and scanned PDFs against their OCR text; spreadsheets against their cell text.
